`timescale 1ns/1ps

module test_tanh_decoder;
    reg clk;
    reg resetn;

    reg         pcpi_valid;
    reg [31:0] pcpi_insn;
    reg [31:0] pcpi_rs1;
    reg [31:0] pcpi_rs2;
    wire        pcpi_wr;
    wire [31:0] pcpi_rd;
    wire        pcpi_wait;
    wire        pcpi_ready;

    wire        decoder_ren_input;
    wire [11:0] decoder_addr_input;
    wire        decoder_ren_weight;
    wire [15:0] decoder_addr_weight;
    wire        decoder_ren_bias;
    wire [9:0]  decoder_addr_bias;
    wire        decoder_pp_ren;
    wire [11:0] decoder_pp_raddr;
    wire        decoder_pp_wen;
    wire [11:0] decoder_pp_waddr;
    wire        decoder_pp_swap;
    wire [15:0] decoder_pp_wdata;
    wire        illegal_instr;

    reg [15:0] pp_mem [0:4095];
    wire [15:0] decoder_pp_rdata = pp_mem[decoder_pp_raddr];

    integer errors;
    integer row;
    integer col;
    integer bit_idx;
    reg [255:0] packed_row;
    reg [15:0] lut_word;

    decoder_acc dut (
        .clk                (clk),
        .resetn             (resetn),
        .pcpi_valid         (pcpi_valid),
        .pcpi_insn          (pcpi_insn),
        .pcpi_rs1           (pcpi_rs1),
        .pcpi_rs2           (pcpi_rs2),
        .pcpi_wr            (pcpi_wr),
        .pcpi_rd            (pcpi_rd),
        .pcpi_wait          (pcpi_wait),
        .pcpi_ready         (pcpi_ready),
        .decoder_ren_input  (decoder_ren_input),
        .decoder_addr_input (decoder_addr_input),
        .decoder_ren_weight (decoder_ren_weight),
        .decoder_addr_weight(decoder_addr_weight),
        .decoder_ren_bias   (decoder_ren_bias),
        .decoder_addr_bias  (decoder_addr_bias),
        .decoder_pp_ren     (decoder_pp_ren),
        .decoder_pp_raddr   (decoder_pp_raddr),
        .decoder_pp_wen     (decoder_pp_wen),
        .decoder_pp_waddr   (decoder_pp_waddr),
        .decoder_pp_swap    (decoder_pp_swap),
        .input_data         (16'h0000),
        .weight_data        (16'h0000),
        .bias_data          (32'h0000_0000),
        .decoder_pp_wdata   (decoder_pp_wdata),
        .decoder_pp_rdata   (decoder_pp_rdata),
        .illegal_instr      (illegal_instr),
        .mem_stall          (1'b0)
    );

    initial begin
        clk = 1'b0;
        forever #5 clk = ~clk;
    end

    function [31:0] acc_insn;
        input [6:0] funct7;
        begin
            acc_insn = {funct7, 5'd0, 5'd0, 3'd0, 5'd0, 7'h2b};
        end
    endfunction

    function [15:0] lut_value;
        input integer addr;
        begin
            lut_value = addr[9:0] << 6;
        end
    endfunction

    task init_lut;
        begin
            for (row = 0; row < 64; row = row + 1) begin
                packed_row = 256'd0;
                for (col = 0; col < 16; col = col + 1) begin
                    lut_word = lut_value(row * 16 + col);
                    for (bit_idx = 0; bit_idx < 16; bit_idx = bit_idx + 1)
                        packed_row[col + bit_idx * 16] = lut_word[bit_idx];
                end
                dut.u_sigmoid.u_sigmoid_lut.mem[row] = packed_row;
            end
            dut.u_sigmoid.u_sigmoid_lut.Q_int = 16'h0000;
        end
    endtask

    task issue_activation;
        input [6:0] funct7;
        input [11:0] pp_addr;
        input signed [15:0] q;
        input [31:0] expected;
        integer timeout;
        begin
            pp_mem[pp_addr] = q[15:0];
            @(negedge clk);
            pcpi_insn  = acc_insn(funct7);
            pcpi_rs1   = {pp_addr, 4'd0, 16'h0001};
            pcpi_rs2   = 32'sd0;
            pcpi_valid = 1'b1;

            timeout = 0;
            while (pcpi_ready !== 1'b1 && timeout < 200) begin
                @(posedge clk);
                timeout = timeout + 1;
            end

            #1;
            if (timeout >= 200) begin
                $display("[TANH_DEC FAIL] funct7=0x%02h timed out", funct7);
                errors = errors + 1;
            end else if (pcpi_wr !== 1'b1 || pcpi_rd !== expected) begin
                $display("[TANH_DEC FAIL] funct7=0x%02h q=%0d expected=0x%08h got=0x%08h wr=%b", funct7, q, expected, pcpi_rd, pcpi_wr);
                errors = errors + 1;
            end else begin
                $display("[TANH_DEC PASS] funct7=0x%02h q=%0d got=0x%08h", funct7, q, pcpi_rd);
            end

            @(negedge clk);
            pcpi_valid = 1'b0;
            pcpi_insn  = 32'd0;
            pcpi_rs1   = 32'd0;
            pcpi_rs2   = 32'd0;
            repeat (4) @(posedge clk);
        end
    endtask

    initial begin
        errors = 0;
        pcpi_valid = 1'b0;
        pcpi_insn = 32'd0;
        pcpi_rs1 = 32'd0;
        pcpi_rs2 = 32'd0;
        init_lut();

        resetn = 1'b0;
        repeat (5) @(posedge clk);
        resetn = 1'b1;
        repeat (2) @(posedge clk);

        issue_activation(7'h0f, 12'd0, 16'sd256, 32'h0000_4000);
        issue_activation(7'h16, 12'd1, 16'sd256, 32'h0000_0000);
        issue_activation(7'h16, 12'd2, 16'sd511, 32'h0000_7f80);
        issue_activation(7'h16, 12'd3, -16'sd1, 32'hffff_8000);

        if (errors == 0) begin
            $display("[TANH_DEC SUCCESS] decoder sigmoid compatibility and tanh opcode passed.");
        end else begin
            $display("[TANH_DEC FAILED] %0d errors.", errors);
        end
        $finish;
    end
endmodule
