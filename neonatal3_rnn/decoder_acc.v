...
wire        instr_tanh              = acc_op && (funct7 == 7'b0010110); // 0x16
...
// RNN/vector-state support instructions. These operate on the ping-pong SRAM
// read/write sides plus a small recurrent state memory inside the decoder.
wire        instr_rnn_pp_to_state   = acc_op && (funct7 == 7'b0010111); // 0x17
wire        instr_rnn_state_to_pp   = acc_op && (funct7 == 7'b0011000); // 0x18
wire        instr_rnn_add_pp_state  = acc_op && (funct7 == 7'b0011001); // 0x19
wire        instr_rnn_mul_pp_state  = acc_op && (funct7 == 7'b0011010); // 0x1A
wire        instr_rnn_one_minus_pp  = acc_op && (funct7 == 7'b0011011); // 0x1B
wire        instr_rnn_write_state   = acc_op && (funct7 == 7'b0011100); // 0x1C
wire        instr_rnn_read_state    = acc_op && (funct7 == 7'b0011101); // 0x1D
wire        instr_rnn_clear_state   = acc_op && (funct7 == 7'b0011110); // 0x1E
wire        instr_rnn_vector_op     = instr_rnn_pp_to_state || instr_rnn_state_to_pp ||
                                      instr_rnn_add_pp_state || instr_rnn_mul_pp_state ||
                                      instr_rnn_one_minus_pp || instr_rnn_clear_state;
...
//------------RNN vector/state FSM signals and helpers---//
localparam RNN_IDLE       = 3'd0;
localparam RNN_READ       = 3'd1;
localparam RNN_WAIT       = 3'd2;
localparam RNN_WRITE      = 3'd3;
localparam RNN_DONE       = 3'd4;
localparam RNN_READ_WAIT  = 3'd5;
localparam RNN_WRITE_WAIT = 3'd6;

localparam RNN_OP_PP_TO_STATE  = 3'd0;
localparam RNN_OP_STATE_TO_PP  = 3'd1;
localparam RNN_OP_ADD_PP_STATE = 3'd2;
localparam RNN_OP_MUL_PP_STATE = 3'd3;
localparam RNN_OP_ONE_MINUS_PP = 3'd4;
localparam RNN_OP_CLEAR_STATE  = 3'd5;
localparam RNN_OP_WRITE_STATE  = 3'd6;
localparam RNN_OP_READ_STATE   = 3'd7;

reg [2:0]  rnn_state;
reg [2:0]  rnn_op;
reg [11:0] rnn_pp_src_base;
reg [11:0] rnn_pp_dst_base;
reg [11:0] rnn_state_src_base;
reg [11:0] rnn_state_dst_base;
reg [11:0] rnn_len_minus1;
reg [11:0] rnn_idx;
reg [11:0] rnn_pp_raddr;
reg        rnn_pp_ren;
reg        rnn_sram_en;
reg        rnn_sram_wen;
reg [11:0] rnn_sram_addr;
reg [15:0] rnn_sram_din;
reg [15:0] rnn_result;
wire [15:0] rnn_sram_dout;

wire [11:0] rnn_state_src_addr = rnn_state_src_base + rnn_idx;
wire [11:0] rnn_state_dst_addr = rnn_state_dst_base + rnn_idx;
...
function [15:0] rnn_sat_signed16;
    input signed [31:0] value;
    begin
        if (value > 32'sd32767)
            rnn_sat_signed16 = 16'h7fff;
        else if (value < -32'sd32768)
            rnn_sat_signed16 = 16'h8000;
        else
            rnn_sat_signed16 = value[15:0];
    end
endfunction

function [15:0] rnn_add_q15;
    input [15:0] a;
    input [15:0] b;
    reg signed [31:0] sum;
    begin
        sum = {{16{a[15]}}, a} + {{16{b[15]}}, b};
        rnn_add_q15 = rnn_sat_signed16(sum);
    end
endfunction

function [15:0] rnn_mul_q15;
    input [15:0] a;
    input [15:0] b;
    ...
    begin
        ...
        rounded = product + 32'sd16384;
        rnn_mul_q15 = rnn_sat_signed16(rounded >>> 15);
    end
endfunction
//-------------------------------------------------------------//
...
wire rnn_active = (rnn_state != RNN_IDLE);
...
assign decoder_pp_raddr = mp_active ? mp_pp_raddr :
                         sig_active ? sig_pp_raddr_r :
                         rnn_active ? rnn_pp_raddr : decoder_addr_input;
assign decoder_pp_ren = mp_active ? mp_pp_ren_r :
                        sig_active ? sig_pp_ren_r :
                        rnn_active ? rnn_pp_ren : decoder_ren_input;
...
always @(posedge clk) begin
    if (!resetn) begin
        ...
        rms_state           <= RMS_IDLE;
        rnn_state           <= RNN_IDLE;
        rnn_op              <= RNN_OP_PP_TO_STATE;
        rnn_pp_src_base     <= 0;
        rnn_pp_dst_base     <= 0;
        rnn_state_src_base  <= 0;
        rnn_state_dst_base  <= 0;
        rnn_len_minus1      <= 0;
        rnn_idx             <= 0;
        rnn_pp_raddr        <= 0;
        rnn_pp_ren          <= 0;
       ...
       if (pcpi_valid && instr_load_ifmap) begin
        ...
        else if (pcpi_valid && (instr_rnn_write_state || instr_rnn_read_state || instr_rnn_vector_op)) begin
            illegal_instr <= 0;
            case (rnn_state)
            RNN_IDLE : begin
                rnn_idx            <= 0;
                ...
                rnn_state_dst_base <= 0;
                rnn_len_minus1     <= 0;
                if (instr_rnn_write_state) begin
                    // rs1[11:0]=state_addr, rs2[15:0]=value
                    rnn_op             <= RNN_OP_WRITE_STATE;
                    rnn_state_dst_base <= pcpi_rs1[11:0];
                    rnn_result         <= pcpi_rs2[15:0];
                    rnn_len_minus1     <= 12'd0;
                end else if (instr_rnn_read_state) begin
                    // rs1[11:0]=state_addr. Read returns sign-extended Q1.15.
                    rnn_op             <= RNN_OP_READ_STATE;
                    rnn_state_src_base <= pcpi_rs1[11:0];
                    rnn_len_minus1     <= 12'd0;
                end else if (instr_rnn_pp_to_state) begin
                    // rs1[11:0]=pp_src, rs1[23:12]=len-1, rs2[11:0]=state_dst
                    rnn_op             <= RNN_OP_PP_TO_STATE;
                    rnn_pp_src_base    <= pcpi_rs1[11:0];
                    rnn_state_dst_base <= pcpi_rs2[11:0];
                    rnn_len_minus1     <= pcpi_rs1[23:12];
                end else if (instr_rnn_state_to_pp) begin
                    // rs1[11:0]=state_src, rs1[23:12]=len-1, rs2[11:0]=pp_dst
                    rnn_op             <= RNN_OP_STATE_TO_PP;
                    rnn_state_src_base <= pcpi_rs1[11:0];
                    rnn_pp_dst_base    <= pcpi_rs2[11:0];
                    rnn_len_minus1     <= pcpi_rs1[23:12];
                end else if (instr_rnn_add_pp_state) begin
                    // rs1[11:0]=pp_src, rs1[23:12]=state_src, rs2[11:0]=pp_dst, rs2[23:12]=len-1
                    rnn_op             <= RNN_OP_ADD_PP_STATE;
                    rnn_pp_src_base    <= pcpi_rs1[11:0];
                    rnn_state_src_base <= pcpi_rs1[23:12];
                    rnn_pp_dst_base    <= pcpi_rs2[11:0];
                    rnn_len_minus1     <= pcpi_rs2[23:12];
                end else if (instr_rnn_mul_pp_state) begin
                    // Same layout as add. Multiplies Q1.15 signed vectors.
                    rnn_op             <= RNN_OP_MUL_PP_STATE;
                    rnn_pp_src_base    <= pcpi_rs1[11:0];
                    rnn_state_src_base <= pcpi_rs1[23:12];
                    rnn_pp_dst_base    <= pcpi_rs2[11:0];
                    rnn_len_minus1     <= pcpi_rs2[23:12];
                end else if (instr_rnn_one_minus_pp) begin
                    // rs1[11:0]=pp_src, rs1[23:12]=len-1, rs2[11:0]=pp_dst
                    rnn_op             <= RNN_OP_ONE_MINUS_PP;
                    rnn_pp_src_base    <= pcpi_rs1[11:0];
                    rnn_pp_dst_base    <= pcpi_rs2[11:0];
                    rnn_len_minus1     <= pcpi_rs1[23:12];
                end else begin
                    // rs1[11:0]=state_dst, rs1[23:12]=len-1
                    rnn_op             <= RNN_OP_CLEAR_STATE;
                    rnn_state_dst_base <= pcpi_rs1[11:0];
                    rnn_len_minus1     <= pcpi_rs1[23:12];
                end
                pcpi_ready <= 0;
            ...
            RNN_READ : begin
                decoder_pp_wen <= 0;
                rnn_sram_wen   <= 0;
                if ((rnn_op == RNN_OP_WRITE_STATE) || (rnn_op == RNN_OP_CLEAR_STATE)) begin
                    if (rnn_op == RNN_OP_CLEAR_STATE) begin
                        rnn_result <= 16'd0;
                    end
                    rnn_state <= RNN_WRITE;
                end else begin
                    if ((rnn_op == RNN_OP_PP_TO_STATE) || (rnn_op == RNN_OP_ADD_PP_STATE) ||
                        (rnn_op == RNN_OP_MUL_PP_STATE) || (rnn_op == RNN_OP_ONE_MINUS_PP)) begin
                        rnn_pp_raddr <= rnn_pp_src_base + rnn_idx;
                        rnn_pp_ren   <= 1;
                    end
                    if ((rnn_op == RNN_OP_STATE_TO_PP) || (rnn_op == RNN_OP_ADD_PP_STATE) ||
                        (rnn_op == RNN_OP_MUL_PP_STATE) || (rnn_op == RNN_OP_READ_STATE)) begin
                        rnn_sram_en   <= 1;
                        rnn_sram_wen  <= 0;
                        rnn_sram_addr <= rnn_state_src_addr;
                    end
                    rnn_state <= RNN_READ_WAIT;
                end
            end
            RNN_READ_WAIT : begin
                rnn_state <= RNN_WAIT;
            ...
            RNN_WAIT : begin
                ...
                rnn_sram_en  <= 0;
                rnn_sram_wen <= 0;
                if (rnn_op == RNN_OP_READ_STATE) begin
                    rnn_result <= rnn_sram_dout;
                    rnn_state  <= RNN_DONE;
                end else if (rnn_op == RNN_OP_STATE_TO_PP) begin
                    rnn_result <= rnn_sram_dout;
                    rnn_state  <= RNN_WRITE;
                end else if (rnn_op == RNN_OP_PP_TO_STATE) begin
                    rnn_result <= decoder_pp_rdata;
                    rnn_state  <= RNN_WRITE;
                end else if (rnn_op == RNN_OP_ADD_PP_STATE) begin
                    rnn_result <= rnn_add_q15(decoder_pp_rdata, rnn_sram_dout);
                    rnn_state  <= RNN_WRITE;
                end else if (rnn_op == RNN_OP_MUL_PP_STATE) begin
                    rnn_result <= rnn_mul_q15(decoder_pp_rdata, rnn_sram_dout);
                    rnn_state  <= RNN_WRITE;
                end else begin
                    rnn_result <= rnn_sat_signed16(32'sd32767 - {{16{decoder_pp_rdata[15]}}, decoder_pp_rdata});
                    rnn_state  <= RNN_WRITE;
                end
            end
            RNN_WRITE : begin
                if ((rnn_op == RNN_OP_PP_TO_STATE) || (rnn_op == RNN_OP_CLEAR_STATE) ||
                    (rnn_op == RNN_OP_WRITE_STATE)) begin
                    rnn_sram_en   <= 1;
                    rnn_sram_wen  <= 1;
                    rnn_sram_addr <= rnn_state_dst_addr;
                    rnn_sram_din  <= rnn_result;
                    decoder_pp_wen <= 0;
                end else begin
                    rnn_sram_en      <= 0;
                    rnn_sram_wen     <= 0;
                    decoder_pp_wen   <= 1;
                    decoder_pp_waddr <= rnn_pp_dst_base + rnn_idx;
                    decoder_pp_wdata <= rnn_result;
                end
                rnn_state <= RNN_WRITE_WAIT;
            end
            RNN_WRITE_WAIT : begin
                decoder_pp_wen <= 0;
                rnn_sram_en    <= 0;
                rnn_sram_wen   <= 0;
                if ((rnn_op == RNN_OP_WRITE_STATE) || (rnn_idx >= rnn_len_minus1)) begin
                    rnn_state <= RNN_DONE;
                end else begin
                    rnn_idx   <= rnn_idx + 1;
                    rnn_state <= RNN_READ;
            ...
            RNN_DONE : begin
                ...
                pcpi_ready     <= 1;
                pcpi_wait      <= 0;
                pcpi_rd        <= (rnn_op == RNN_OP_READ_STATE) ? {{16{rnn_result[15]}}, rnn_result} : 32'd0;
                pcpi_wr        <= (rnn_op == RNN_OP_READ_STATE);
                rnn_state      <= RNN_DONE;
            end
            default : begin
                rnn_state <= RNN_IDLE;
        ...
        else if(pcpi_valid && (instr_sigmoid || instr_tanh)) begin
            illegal_instr <= 0;
            case(sig_state)
            SIG_IDLE : begin
            ...
            SIG_DONE : begin
                pcpi_ready  <= 1;
                pcpi_wait   <= 0;
                pcpi_rd     <= sig_tanh_mode ? {{16{sig_out[15]}}, sig_out} : {16'h0000, sig_out};
                pcpi_wr     <= 1;
