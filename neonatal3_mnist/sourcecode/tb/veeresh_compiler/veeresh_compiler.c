...
int main(void) {
    ...
    /* --- LAYER_1_CONV --- */
    acc_swap_pp(0U);
    for(uint32_t p=0; p<8; p++) { acc_load_qat(p, 0x477BC17EU, (((uint32_t)0x29 << 16) | 0x0000U)); }
    { uint32_t ws = 0U, bias_ptr = 0U, out_ptr = 0U;
      for (uint32_t b=0; b<1U; b++) {
          run_conv_batch_generic(ws, bias_ptr, out_ptr, 28U, 3U, 28U, 8U, 1U, 1U);
          ws += 672U; bias_ptr += 8U; out_ptr += 8U; } }

    /* --- LAYER_2_POOL --- */
    acc_swap_pp(1U);
    { uint32_t rd=0U, wr=0U, stride=8U, ps=14U;
      acc_maxpool((stride << 12) | rd, (ps << 12) | wr); 
    }
    /* --- LAYER_3_CONV --- */
    acc_swap_pp(1U);
    for(uint32_t p=0; p<8; p++) { acc_load_qat(p, 0x61760763U, (((uint32_t)0x26 << 16) | 0x0000U)); }
    { uint32_t ws = 672U, bias_ptr = 8U, out_ptr = 0U;
      for (uint32_t b=0; b<2U; b++) {
          run_conv_batch_generic(ws, bias_ptr, out_ptr, 8U, 3U, 14U, 16U, 1U, 1U);
          ws += 192U; bias_ptr += 8U; out_ptr += 8U; } }

    /* --- LAYER_4_POOL --- */
    acc_swap_pp(1U);
    { uint32_t rd=0U, wr=0U, stride=16U, ps=7U;
      acc_maxpool((stride << 12) | rd, (ps << 12) | wr); 
    }
    /* --- LAYER_5_DENSE --- */
    acc_swap_pp(1U);
    for(uint32_t p=0; p<8; p++) { acc_load_qat(p, 0x4307C742U, (((uint32_t)0x26 << 16) | 0x0000U)); }
    run_dense_batch(1056U, 24U, 0U, 111U, 8U, 1U);
    run_dense_batch(1952U, 32U, 8U, 111U, 8U, 1U);
    run_dense_batch(2848U, 40U, 16U, 111U, 8U, 1U);
    run_dense_batch(3744U, 48U, 24U, 111U, 8U, 1U);

    /* --- LAYER_6_DENSE --- */
    acc_swap_pp(1U);
    for(uint32_t p=0; p<8; p++) { acc_load_qat(p, 0x506BAA97U, (((uint32_t)0x27 << 16) | 0x008DU)); }
    run_dense_batch(4640U, 56U, 0U, 31U, 8U, 0U);
    run_dense_batch(4896U, 64U, 8U, 31U, 2U, 0U);
