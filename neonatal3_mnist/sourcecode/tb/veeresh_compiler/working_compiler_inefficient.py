...
def generate_conv_c(name, hw, in_ch, out_ch, k_size, S_in, S_w, S_out, Z_out, Z_in, is_first_layer=False):
    ...
    # Call the new Universal Macro!
    c += f"          run_conv_batch_generic(ws, bias_ptr, out_ptr, {in_ch}U, {k_size}U, {hw.seq_len}U, {out_ch}U, {pad_l}U);\n"
...
static void run_conv_batch_generic(uint32_t w_start_base, uint32_t bias_offset, uint32_t out_offset, 
                                   uint32_t in_ch, uint32_t k_size, uint32_t seq_len, uint32_t out_ch, uint32_t pad_l) {
    uint32_t p, t;
    uint32_t pp_base = out_offset;
    
    // Replaced '*' with soft_mul()
...
"""
    ...
    for block in c_blocks[0:8]: c_firmware += block
