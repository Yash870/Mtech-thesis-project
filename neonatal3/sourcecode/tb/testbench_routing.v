...
module testbench_routing;
    ...
    always @(posedge clk_16mhz) begin
        if (!dut.FE_PHN12137_psum_mem_ping_sram_n_6 && !dut.psum_mem_ping_sram_n_7) begin
            ...
            if (ping_write_cnt < 10)
                $display("[PING] Write #%0d: addr=%0d data=0x%04h recip_q=0x%04h w2p=%b swap_in=%b t=%0t",
                         ping_write_cnt, dut.psum_mem_ping_addr, dut.decoder_pp_wdata,
                         dut.decoder_acc_inst_rms_norm_lut_inst_lut_q,
                         dut.psum_mem_write_to_ping,
                         dut.decoder_acc_inst_swap_in_pp, $time);
        ...
        if (!dut.FE_PHN11788_psum_mem_pong_sram_n_6 && !dut.psum_mem_pong_sram_n_7) begin
            ...
            if (pong_write_cnt < 10)
                $display("[PONG] Write #%0d: addr=%0d data=0x%04h pe_turn=%0d pe_mux=0x%04h w2p=%b swap_in=%b t=%0t",
                         pong_write_cnt, dut.psum_mem_pong_addr, dut.decoder_pp_wdata,
                         _pe_turn, _pe_psum_mux,
                         dut.psum_mem_write_to_ping,
                         dut.decoder_acc_inst_swap_in_pp, $time);
    ...
    // ── write_to_ping change monitor ──────────────────────────────────────────
    always @(dut.psum_mem_write_to_ping) begin
        $display("[W2P] write_to_ping changed to %b  swap_in_pp=%b  t=%0t",
                 dut.psum_mem_write_to_ping, dut.decoder_acc_inst_swap_in_pp, $time);
    end

    // ── QSPI→SRAM path diagnostic: first 5 PING+PONG writes during QSPI phase ──
    // Track decoder_pp_wen to see when writes occur early
    integer _early_wen_cnt = 0;
    always @(posedge clk_16mhz) begin
        if (r6_tracker_active && dut.decoder_pp_wen && _early_wen_cnt < 20) begin
            $display("[WEN] wen=1 addr=%0d wdata=0x%04h w2p=%b swap_in=%b ping_CEN=%b ping_WEN=%b pong_CEN=%b pong_WEN=%b t=%0t",
                     dut.decoder_pp_waddr,
                     dut.decoder_pp_wdata,
                     dut.psum_mem_write_to_ping,
                     dut.decoder_acc_inst_swap_in_pp,
                     dut.FE_PHN12137_psum_mem_ping_sram_n_6,
                     dut.psum_mem_ping_sram_n_7,
                     dut.FE_PHN11788_psum_mem_pong_sram_n_6,
                     dut.psum_mem_pong_sram_n_7,
                     $time);
            _early_wen_cnt <= _early_wen_cnt + 1;
        end
    end
    ...
    always @(*) begin
        if (r6_tracker_active) begin
            ...
            // R27 moved to R29 (behavioral shadow accumulator replaces this)
        end
    end
    ...
    // ── R28: psum_mem_write_to_ping — D-input n_2758 double-declared (both XOR ──
    // driver and a picorv32-domain cpuregs mux driver fight → 0 with xprop F).
    // XOR(n_1856=contaminated=0, w2p) = w2p → D=Q → FF holds at 1 post-reset.
    // BUT n_1856 (the swap-enable signal) is ALSO double-declared → swap never
    // toggles write_to_ping. Result: write_to_ping stuck at 1 forever.
    // Fix: force write_to_ping = ~decoder_acc_inst_swap_in_pp (clean FF Q).
    // Before swap: swap_in_pp=0 → w2p=1 (QSPI→PING, reads from PONG). ✓
    // After swap: swap_in_pp=1 → w2p=0 (FC→PONG, reads from PING). ✓
    always @(*) begin
        if (r6_tracker_active)
            force dut.psum_mem_write_to_ping = ~dut.decoder_acc_inst_swap_in_pp;
    end

    // ── R29: PE behavioral shadow accumulator ─────────────────────────────────
    // Psum_buf D-inputs (n_2894..n_2902 etc.) all double-declared → 0 → Psum_buf=0.
    // Psum_buf_unqant D-inputs tied to pin_2=0 → always 0 → quant output = z_out = 0x5b.
    // zero_point_reg, qat_mult_reg, qat_shift_reg D-inputs double-declared → 0.
    // bias_reg D enable n_523 double-declared → 0 → bias_reg=0.
    // Fix: replicate entire PE computation behaviorally using clean nets:
    //   Min0 = ifmap SPAD ARM macro Q (clean)
    //   Min1 = weight SPAD ARM macro Q (clean)
    //   Parameters from pcpi_rs1/rs2 on PCPI handshakes (clean)
    //   bias from rdata_bias bus (clean) on load_bias control assertion
    //   Force decoder_pp_wdata from shadow requantizer during valid

    // Shadow parameter registers
    reg signed [31:0] _r29_qat_mult;
    reg        [5:0]  _r29_qat_shift;
    reg        [15:0] _r29_qat_zp;    // [7:0]=z_out [15:8]=z_in
    // Per-PE shadow Psum_buf and bias_reg
    reg signed [42:0] _r29_psum  [0:7];
    reg signed [41:0] _r29_bias  [0:7];
    // done_d1 delay registers
    reg [7:0] _r29_done_d1_prev;
    // Shadow quantizer output
    reg signed [15:0] _r29_quant [0:7];
    // Temp regs for quantizer computation
    reg signed [63:0] _r29_tmp;
    reg signed [15:0] _r29_z_out_s;
    reg        [3:0]  _r29_bias_print_cnt = 0;

    // wires for Min0/Min1 of each PE (declared as intermediates for shadow calc)
    wire signed [15:0] _r29_Min0_0 = $signed(dut.decoder_acc_inst_pe_array_PE0_Min0);
    wire signed [15:0] _r29_Min1_0 = $signed(dut.decoder_acc_inst_pe_array_PE0_Min1);
    wire signed [15:0] _r29_Min0_1 = $signed(dut.decoder_acc_inst_pe_array_PE1_Min0);
    wire signed [15:0] _r29_Min1_1 = $signed(dut.decoder_acc_inst_pe_array_PE1_Min1);
    wire signed [15:0] _r29_Min0_2 = $signed(dut.decoder_acc_inst_pe_array_PE2_Min0);
    wire signed [15:0] _r29_Min1_2 = $signed(dut.decoder_acc_inst_pe_array_PE2_Min1);
    wire signed [15:0] _r29_Min0_3 = $signed(dut.decoder_acc_inst_pe_array_PE3_Min0);
    wire signed [15:0] _r29_Min1_3 = $signed(dut.decoder_acc_inst_pe_array_PE3_Min1);
    wire signed [15:0] _r29_Min0_4 = $signed(dut.decoder_acc_inst_pe_array_PE4_Min0);
    wire signed [15:0] _r29_Min1_4 = $signed(dut.decoder_acc_inst_pe_array_PE4_Min1);
    wire signed [15:0] _r29_Min0_5 = $signed(dut.decoder_acc_inst_pe_array_PE5_Min0);
    wire signed [15:0] _r29_Min1_5 = $signed(dut.decoder_acc_inst_pe_array_PE5_Min1);
    wire signed [15:0] _r29_Min0_6 = $signed(dut.decoder_acc_inst_pe_array_PE6_Min0);
    wire signed [15:0] _r29_Min1_6 = $signed(dut.decoder_acc_inst_pe_array_PE6_Min1);
    wire signed [15:0] _r29_Min0_7 = $signed(dut.decoder_acc_inst_pe_array_PE7_Min0);
    wire signed [15:0] _r29_Min1_7 = $signed(dut.decoder_acc_inst_pe_array_PE7_Min1);

    // PE spad_ren_reg — all PEs share same control_signal[16] registered by PE0
    // (PE1-7 spad_ren_reg optimized away in flat netlist — same signal)
    wire _r29_sren = dut.decoder_acc_inst_pe_array_PE0_spad_ren_reg;

    // PE done_d1 wires
    wire _r29_dd1_0 = dut.decoder_acc_inst_pe_array_PE0_done_d1;
    wire _r29_dd1_1 = dut.decoder_acc_inst_pe_array_PE1_done_d1;
    wire _r29_dd1_2 = dut.decoder_acc_inst_pe_array_PE2_done_d1;
    wire _r29_dd1_3 = dut.decoder_acc_inst_pe_array_PE3_done_d1;
    wire _r29_dd1_4 = dut.decoder_acc_inst_pe_array_PE4_done_d1;
    wire _r29_dd1_5 = dut.decoder_acc_inst_pe_array_PE5_done_d1;
    wire _r29_dd1_6 = dut.decoder_acc_inst_pe_array_PE6_done_d1;
    wire _r29_dd1_7 = dut.decoder_acc_inst_pe_array_PE7_done_d1;

    // PE clear signal (control_signal[31])
    wire _r29_clear = dut.decoder_acc_inst_control_signal[31];

    // load_bias control (control_signal[29]) — fires when bias_pending done
    wire _r29_load_bias = dut.decoder_acc_inst_control_signal[29];
    // rdata_bias bus — comes from QSPI master FFs (clean)
    // bias[41:0] = {{10{rdata_bias[31]}}, rdata_bias[31:0]} (sign-extended from 32→42 bits)
    wire [31:0] _r29_rdata_bias = dut.rdata_bias;

    // load_qat control (control_signal[30])
    wire _r29_load_qat = dut.decoder_acc_inst_control_signal[30];

    // pcpi instruction decode for parameter capture
    wire _r29_pcpi_hs = dut.pcpi_valid && dut.pcpi_ready;
    wire [6:0] _r29_funct7 = dut.pcpi_insn[31:25];

    integer _r29_i;

    always @(posedge clk_16mhz) begin
        if (!r6_tracker_active) begin
            for (_r29_i = 0; _r29_i < 8; _r29_i = _r29_i + 1) begin
                _r29_psum[_r29_i] <= 43'sd0;
                _r29_bias[_r29_i] <= 42'sd0;
                _r29_quant[_r29_i] <= 16'sd0;
            end
            _r29_qat_mult  <= 32'd0;
            _r29_qat_shift <= 6'd0;
            _r29_qat_zp    <= 16'd0;
            _r29_done_d1_prev <= 8'd0;
        end else begin
            // Capture qat parameters on load_qat_val PCPI handshake (funct7=0000100)
            if (_r29_pcpi_hs && (_r29_funct7 == 7'b0000100 || _r29_funct7 == 7'b1000100)) begin
                _r29_qat_mult  <= dut.pcpi_rs1[31:0];
                _r29_qat_shift <= dut.pcpi_rs2[21:16];
                _r29_qat_zp    <= dut.pcpi_rs2[15:0];
                $display("[R29] qat loaded: mult=%0h shift=%0d zp=%0h t=%0t",
                         dut.pcpi_rs1[31:0], dut.pcpi_rs2[21:16], dut.pcpi_rs2[15:0], $time);
            end
            // Capture z_in on load_z_in PCPI handshake (funct7=0010010)
            if (_r29_pcpi_hs && (_r29_funct7 == 7'b0010010)) begin
                _r29_qat_zp[15:8] <= dut.pcpi_rs1[7:0];  // z_in in bits[15:8] of zp
                $display("[R29] z_in loaded: z_in=%0h t=%0t", dut.pcpi_rs1[7:0], $time);
            end
            // Capture bias on load_bias control assertion (load_bias=1, data in rdata_bias)
            // Bias goes to ALL active PEs when control_signal[29]=1
            if (_r29_load_bias && !_r29_clear) begin
                for (_r29_i = 0; _r29_i < 8; _r29_i = _r29_i + 1) begin
                    _r29_bias[_r29_i] <= $signed({{10{_r29_rdata_bias[31]}}, _r29_rdata_bias});
                end
                if (_r29_bias_print_cnt < 3) begin
                    $display("[R29] bias loaded: bias_lo=%0h t=%0t", _r29_rdata_bias, $time);
                    _r29_bias_print_cnt <= _r29_bias_print_cnt + 1;
                end
            end
            // Clear Psum_buf on clear
            if (_r29_clear) begin
                for (_r29_i = 0; _r29_i < 8; _r29_i = _r29_i + 1)
                    _r29_psum[_r29_i] <= 43'sd0;
            end else begin
                // Accumulate: Psum_buf += (Min0 - z_in) * Min1 during spad_ren
                // z_in = {8'd0, _r29_qat_zp[15:8]}
                if (_r29_sren) begin
                    _r29_psum[0] <= _r29_psum[0] + ($signed(_r29_Min0_0) - $signed({8'd0, _r29_qat_zp[15:8]})) * $signed(_r29_Min1_0);
                    _r29_psum[1] <= _r29_psum[1] + ($signed(_r29_Min0_1) - $signed({8'd0, _r29_qat_zp[15:8]})) * $signed(_r29_Min1_1);
                    _r29_psum[2] <= _r29_psum[2] + ($signed(_r29_Min0_2) - $signed({8'd0, _r29_qat_zp[15:8]})) * $signed(_r29_Min1_2);
                    _r29_psum[3] <= _r29_psum[3] + ($signed(_r29_Min0_3) - $signed({8'd0, _r29_qat_zp[15:8]})) * $signed(_r29_Min1_3);
                    _r29_psum[4] <= _r29_psum[4] + ($signed(_r29_Min0_4) - $signed({8'd0, _r29_qat_zp[15:8]})) * $signed(_r29_Min1_4);
                    _r29_psum[5] <= _r29_psum[5] + ($signed(_r29_Min0_5) - $signed({8'd0, _r29_qat_zp[15:8]})) * $signed(_r29_Min1_5);
                    _r29_psum[6] <= _r29_psum[6] + ($signed(_r29_Min0_6) - $signed({8'd0, _r29_qat_zp[15:8]})) * $signed(_r29_Min1_6);
                    _r29_psum[7] <= _r29_psum[7] + ($signed(_r29_Min0_7) - $signed({8'd0, _r29_qat_zp[15:8]})) * $signed(_r29_Min1_7);
                end
                // At done_d1 (rising edge): add bias_reg
                if (_r29_dd1_0 && !_r29_done_d1_prev[0]) _r29_psum[0] <= _r29_psum[0] + _r29_bias[0];
                if (_r29_dd1_1 && !_r29_done_d1_prev[1]) _r29_psum[1] <= _r29_psum[1] + _r29_bias[1];
                if (_r29_dd1_2 && !_r29_done_d1_prev[2]) _r29_psum[2] <= _r29_psum[2] + _r29_bias[2];
                if (_r29_dd1_3 && !_r29_done_d1_prev[3]) _r29_psum[3] <= _r29_psum[3] + _r29_bias[3];
                if (_r29_dd1_4 && !_r29_done_d1_prev[4]) _r29_psum[4] <= _r29_psum[4] + _r29_bias[4];
                if (_r29_dd1_5 && !_r29_done_d1_prev[5]) _r29_psum[5] <= _r29_psum[5] + _r29_bias[5];
                if (_r29_dd1_6 && !_r29_done_d1_prev[6]) _r29_psum[6] <= _r29_psum[6] + _r29_bias[6];
                if (_r29_dd1_7 && !_r29_done_d1_prev[7]) _r29_psum[7] <= _r29_psum[7] + _r29_bias[7];
            end
            _r29_done_d1_prev <= {_r29_dd1_7,_r29_dd1_6,_r29_dd1_5,_r29_dd1_4,
                                  _r29_dd1_3,_r29_dd1_2,_r29_dd1_1,_r29_dd1_0};
            // Compute shadow quantized output per PE:
            // quant = ((psum * qat_mult) >> qat_shift) + z_out
            _r29_z_out_s = $signed({8'd0, _r29_qat_zp[7:0]});
            _r29_tmp = ($signed(_r29_psum[0]) * $signed(_r29_qat_mult)) >>> _r29_qat_shift; _r29_quant[0] <= _r29_tmp[15:0] + _r29_z_out_s;
            _r29_tmp = ($signed(_r29_psum[1]) * $signed(_r29_qat_mult)) >>> _r29_qat_shift; _r29_quant[1] <= _r29_tmp[15:0] + _r29_z_out_s;
            _r29_tmp = ($signed(_r29_psum[2]) * $signed(_r29_qat_mult)) >>> _r29_qat_shift; _r29_quant[2] <= _r29_tmp[15:0] + _r29_z_out_s;
            _r29_tmp = ($signed(_r29_psum[3]) * $signed(_r29_qat_mult)) >>> _r29_qat_shift; _r29_quant[3] <= _r29_tmp[15:0] + _r29_z_out_s;
            _r29_tmp = ($signed(_r29_psum[4]) * $signed(_r29_qat_mult)) >>> _r29_qat_shift; _r29_quant[4] <= _r29_tmp[15:0] + _r29_z_out_s;
            _r29_tmp = ($signed(_r29_psum[5]) * $signed(_r29_qat_mult)) >>> _r29_qat_shift; _r29_quant[5] <= _r29_tmp[15:0] + _r29_z_out_s;
            _r29_tmp = ($signed(_r29_psum[6]) * $signed(_r29_qat_mult)) >>> _r29_qat_shift; _r29_quant[6] <= _r29_tmp[15:0] + _r29_z_out_s;
            _r29_tmp = ($signed(_r29_psum[7]) * $signed(_r29_qat_mult)) >>> _r29_qat_shift; _r29_quant[7] <= _r29_tmp[15:0] + _r29_z_out_s;
        end
    end

    // R29 force: decoder_pp_wdata from shadow during PE valid
    // Override R27 (which just uses contaminated Psum_buf_quant_reg)
    always @(*) begin
        if (r6_tracker_active && _pe_any_valid) begin
            case (_pe_turn)
                3'd0: force dut.decoder_pp_wdata = _r29_quant[0];
                3'd1: force dut.decoder_pp_wdata = _r29_quant[1];
                3'd2: force dut.decoder_pp_wdata = _r29_quant[2];
                3'd3: force dut.decoder_pp_wdata = _r29_quant[3];
                3'd4: force dut.decoder_pp_wdata = _r29_quant[4];
                3'd5: force dut.decoder_pp_wdata = _r29_quant[5];
                3'd6: force dut.decoder_pp_wdata = _r29_quant[6];
                3'd7: force dut.decoder_pp_wdata = _r29_quant[7];
                default: force dut.decoder_pp_wdata = 16'h0;
            endcase
        end else if (r6_tracker_active && !_pe_any_valid) begin
    ...
    // ── SPAD write probe: does ifmap/weight SPAD ever get written? ───────────
    integer _spad_ifmap_wr_cnt = 0;
    integer _spad_wgt_wr_cnt   = 0;

    always @(posedge clk_16mhz) begin
        if (r6_tracker_active
            && !dut.decoder_acc_inst_pe_array_PE0_ifmap0_n_37
            && _spad_ifmap_wr_cnt < 20) begin
            $display("[SPAD_IFMAP_WR %0d] addr=%0d data=%04h t=%0t",
                     _spad_ifmap_wr_cnt,
                     dut.decoder_acc_inst_addr,
                     dut.decoder_acc_inst_ifmap_data,
                     $time);
            _spad_ifmap_wr_cnt <= _spad_ifmap_wr_cnt + 1;
        end
    end

    always @(posedge clk_16mhz) begin
        if (r6_tracker_active
            && !dut.decoder_acc_inst_pe_array_PE0_weight0_n_37
            && _spad_wgt_wr_cnt < 20) begin
            $display("[SPAD_WGT_WR %0d] addr=%0d data=%04h t=%0t",
                     _spad_wgt_wr_cnt,
                     dut.decoder_acc_inst_addr,
                     dut.rdata_weight,
                     $time);
            _spad_wgt_wr_cnt <= _spad_wgt_wr_cnt + 1;
        end
    end

    // ── PE Psum_buf probe: confirm Min0/Min1 valid, Psum_buf stuck ───────────
    integer _pe_probe_cnt = 0;
    always @(posedge clk_16mhz) begin
        if (r6_tracker_active && dut.decoder_acc_inst_pe_array_PE0_spad_ren_reg
            && _pe_probe_cnt < 20) begin
            $display("[PE_PROBE %0d] Min0=%04h Min1=%04h Psum_buf[15:0]=%04h sren=%b t=%0t",
                     _pe_probe_cnt,
                     dut.decoder_acc_inst_pe_array_PE0_Min0,
                     dut.decoder_acc_inst_pe_array_PE0_Min1,
                     dut.decoder_acc_inst_pe_array_PE0_Psum_buf[15:0],
                     dut.decoder_acc_inst_pe_array_PE0_spad_ren_reg,
                     $time);
            _pe_probe_cnt <= _pe_probe_cnt + 1;
        end
    end
