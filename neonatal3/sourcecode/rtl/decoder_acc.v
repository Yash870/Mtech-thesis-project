...
wire        instr_load_ifmap        = acc_op && (funct7 == 7'b0000001 || funct7 == 7'b1000001); 
wire        instr_load_weight       = acc_op && (funct7 == 7'b0000010 || funct7 == 7'b1000010); 
wire        instr_load_bias         = acc_op && (funct7 == 7'b0000011); 
    ...
    if (!resetn) begin
       ...
       if (pcpi_valid && instr_load_ifmap) begin
            ...
            if(done_input || done_ifmap_pending) begin
                ...
                control_signal      <= {16'h0000,8'h40,4'h0,{1'b0,funct3}}| (rms_en ? 32'h0004_0000 : 32'h0);
                enable_input        <= 0;
                decoder_ren_input   <= 0;
                if (!mem_stall) begin
                    pcpi_ready          <= 1;
            ...
            if(done_weight || done_weight_pending) begin
                // Wait for last QSPI fetch to complete before releasing CPU
                control_signal      <= {16'h0000,8'h80,4'h0,{1'b0,funct3}}| (rms_en ? 32'h0004_0000 : 32'h0);
                enable_weight       <= 0;
                decoder_ren_weight  <= 0;
                if (!mem_stall) begin
                    pcpi_ready           <= 1;
            ...
            control_signal    <= {16'h2000,8'h00,4'h0,{1'b0,funct3}};
            pcpi_wr           <= 0;
            pcpi_rd           <= 0;
